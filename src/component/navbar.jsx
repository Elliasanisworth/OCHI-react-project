...
function Navbar() {
  ...
  return (
    <div
      className={`fixed z-[999] w-full px-20 py-8 font-['neue Montreal'] flex justify-between items-center transition-transform duration-300 ${scrollingUp ? 'translate-y-0' : '-translate-y-full'}  backdrop-blur-md`}
    >
      <div className='logo'>
        {/* SVG Logo */}
      ...
      <div className='links flex gap-10 relative'>
        {["services", "Our Work", "About Us", "Insights", "Contact Us"].map((item, index) => (
          <a
            key={index}
            className={`text-lg capitalize font-light relative group`}
          >
            {item}
            <span className="absolute left-0 bottom-[-5px] w-full h-[1.5px] bg-zinc-800 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300"></span>
          </a>
        ))}
      </div>
    </div>
